\section{Charged Particle Motion}
\label{sec:motion}
...
\begin{center}
    ...
    \begin{tabular}{c c c}
        ...
        \input{../pictures/theory/Electric-Force.tex} & \hspace{-1cm}
        \input{../pictures/theory/Inhomogenous-Magnetic-Field.tex}\\[0.3cm]
        \textbf{(a)} & \textbf{(b)} & \textbf{(c)} \\
    \end{tabular}
    \captionof{figure}{Forces acting on a charged particle:\\
        \begin{tabular}{l l}
            \textbf{(a)} & Circular motion caused by Lorentz force $\vect{F}^s_{\perp, B}$  \\
                         & with perpendicular velocity $\vect{v}_{\perp, s}$ and Larmor radius $\rho_s$, \\
            \textbf{(b)} & Electric force $\vect{F}_{\parallel,E}^s$ with electric field $\vect{E}$,\\
...
\begin{center}
    ...
    \captionof{figure}{Dirft motion in gyrocenter:\\
        \begin{tabular}{l l}
            ...
            \textbf{(b)} & $\nabla B$ Drift/Curvature Drift with drift velocity $\vect{v}^{s}_{\nabla B;C}$, magnetic\\
