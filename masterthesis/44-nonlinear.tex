...
To calculate the nonlinear terms with {\gkw} the following changes to the code are applied:
\begin{itemize}
    \item Since the introduction of the f-version of {\gkw}, the solution array \code{fdisi} stores $\df$, so the $\Apar$-term has to be added to distribution stored in \code{fdisi}, i.e. apply the $\Apar$-correction. The $\Apar$-correction to receive $\df$ from $g$ is already implemented into {\gkw} with the matrix \code{matg2f} with its elements defined in the subroutine \code{apar\_correct}. To apply the $\Apar$-correction to $\df$ the signs of the matrix element of \code{matg2f} have to be swaped. To distinguish, between the f-version and the g-version the matrix elements are written into the matrix \code{matf2g} instead of using \code{matg2f}.
...
\end{itemize}
The changes are valid if the input parameter \code{f\_version} and \code{non\_linear} are active and local simulations are performed.
