...
\begin{tikzpicture}
	...
	\node [Notagreen] at (0.26*\W,0.75*\H) {$\vect{B}$};
	% Force
	\draw [thick, Notagrey, ->, >=stealth] ( 95:{\Rx} and {\Ry}) --++ (-25:0.8*\Ry) node[above right=0, Notagrey] {$\vect{F}_{\parallel,\nabla_{\!\parallel} B}$};
	\draw [thick, Notagrey, ->, >=stealth] (-95:{\Rx} and {\Ry}) --++ ( 25:0.8*\Ry) node[below right=1, Notagrey] {$\vect{F}_{\parallel,\nabla_{\!\parallel} B}$};
